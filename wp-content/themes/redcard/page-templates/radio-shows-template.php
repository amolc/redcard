...
<div class="box">
				...
				<?php
				    	...
				    	$args = array(
	 					...
	 					$selectsql = "SELECT *  FROM `rd_postmeta` WHERE `meta_key` = '_cmb_radioshows_from_time'";
						$metafrom = $wpdb->get_results($selectsql) ;
						$metafrom[0]->meta_value;
						
						$selectsqlto = "SELECT *  FROM `rd_postmeta` WHERE `meta_key` = '_cmb_radioshows_to_time'";
						$metato = $wpdb->get_results($selectsqlto) ;
						$metato[0]->meta_value;
						  //echo $metafrom[0]['meta_value']; die(); 
        ...
        <a href="http://twitter.com/intent/tweet?text=<?php echo $article->post_title.get_permalink( $article->ID);?> via@RedCardConnect&url=" target="_blank" onclick="javascript:window.open(this.href,'', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=400,width=400');return false;"><div class="twitter"></div></a>
