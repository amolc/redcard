...
Plugin URI: http://www.glockeasymail.com/newsletter-plugin-wordpress/
Description: G-Lock Double Opt-In Manager is discontinued and will not be updated anymore. It's replaced with G-Lock WPNewsman plugin that can be downloaded at this page <a href="http://wpnewsman.com/">WPNewsman.com</a>. Please export your subscribers and import them into WPNewsman - your new newsletter app for WordPress.
Version: 2.6.5
...
$gsom_filter_images = '1';
		...
		$res = gsom_version_compare($old_db_version,'2.6.0');   
		if ($res === GSOM_VER_LOWER) {
			// adding
			update_option('gsom_support_link', 0);
		}     
	...
	global $gsom_form_vars;
		...
		if (isset($_REQUEST['submit']))
		{
			 ...
			 gsom_rearrange_fields($gsom_form);
				...
				$supportEnabled = isset( $_POST['gsom_support_link'] ) ? 1 : 0;
				update_option('gsom_support_link', $supportEnabled);
...
?>      
<p>
If you find any bugs or have ideas for improvements or enhancements to functionality, please tell us. If possible, we will take your hints in consideration for future releases. If you have any questions or problems and want to get help from our support team or other users, please, visit <a target="_blank" href="http://www.justlan.com/forum/viewforum.php?f=24">G-Lock Software Community Forums</a>. We will be proud to help you. We will be happy if you could suggest WordPress Newsletter Plugin from G-Lock Software to other people who could be interested in it, e. g. your friends, colleagues or maybe even your website visitors. Download <a href="http://www.glockeasymail.com/">G-Lock EasyMail Free version</a> to send your newsletters.
</p>
		...
		<div id="gsom-page-general" class="gsom-tbl-wrapper" <?php echo $gsom_tab != 'gsom-page-general' ? 'style="display:none;"' : ''?>>    
				<table class="form-table">
					...
					<input class="gsom_bnone" style="float:left; margin-right:5px; margin-top: 5px;" type="checkbox" name="gsom_write_debug_log" value="1" <?php echo $gsom_write_debug_log ? 'checked="checked"' : ''?> /><label style="float:left; line-height:22px; vertical-align:middle;" for="gsom_write_debug_log">Write Debug Log</label><br style="clear:both;" />                	
						...
						<tr>
								<th scope="row" class="gsom-sec-hdr">Plugin support</th>
								<td valign="top" style="clear:both">
									<input class="gsom_bnone" style="float:left; margin-right:5px; margin-top: 5px;" type="checkbox" name="gsom_support_link" value="1" <?php echo get_option('gsom_support_link') ? 'checked="checked"' : ''?> /><label style="float:left; line-height:22px; vertical-align:middle;" for="gsom_support_link">Enable support link</label><br style="clear:both;" />
									<p class="info">This option will place a small line that says <code>Newsletter plugin by <a href="http://glockeasymail.com">glockeasymail.com</a></code> underneath the subscription form. This will support future development of G-Lock Opt-in plugin.</p>
									<p>Alternatively, you can make a donation by folowing <a href="https://www.iportis.com/buynow.php?pid=yb8n8g1vyruvyxaw6q2m" target="_blank">this link</a>  or place a link in some other place on your site (e.g. credits page, write a review post for this plugin on your blog, etc.).</p>
								</td>
								<td class="info">                  
								</td>                
						</tr>
		...
		<table class="form-table">
				...
				<tr>
						...
						<td class="info">
								...
								<p>In this article you'll find <a target="_blank" href="http://www.glockeasymail.com/3-tips-for-generating-profits-from-the-signup-process/">3 Tips for Generating Profits from the Signup Process</a></p>            	
						</td>
				</tr>    
		</table>
		</div>
...
<div style="border-bottom: 1px solid #DADADA; overflow: hidden;">
		<h2 style="border:none; padding:0; margin: 10px 0;">Manage Subscribers

			<div style="float:right; line-height: 28px; font-size: 12px; vertical-align: middle;">
...
You can <a href="http://www.glockeasymail.com/tutorials/import-subscribers-wordpress-database-glock-easymail/">connect directly to your WordPress subscribers from G-Lock EasyMail 6 address book</a> and send an personalized email to them without the need to export-import the list
</div>
...
You can merge the varUCode field into the unsubscribe link and use it in <a href="http://www.glockeasymail.com/">G-Lock EasyMail</a> as shown above.
</div>
</div>
<script type="text/javascript">
	glock.event.observe(window,'load',function(){
...
<div class="wrap">
<!-- <div style="border-bottom: 1px solid #DADADA; overflow: hidden;"> -->
	<h2 style="padding:0; margin: 10px 0;">Blog Broadcast Settings</h2>
<!-- </div> -->
...
//*/    
	if (get_option('gsom_transform') == '1') {
		...
		if (function_exists('wp_enqueue_script')) {
				...
				if (defined('WP_ADMIN')) {
					# Admin may be using secure HTTP!!
					if (function_exists('is_ssl') and is_ssl()) {
						$base = str_replace('http:', 'https:',
							$gsom_form_vars['wp_url']);
					} else {
						$base = $gsom_form_vars['wp_url'];
					}
					wp_register_script('glock', $base.PLUGINDIR.'/'.dirname(plugin_basename(__FILE__)) . '/js/glock2.min.js', array(), $gsom_db_version);
					wp_enqueue_script('glock');        
					
					wp_register_script('gsom_s', $base.PLUGINDIR.'/'.dirname(plugin_basename(__FILE__)) . '/js/gsom_s.min.js', array(), $gsom_db_version);
	...
	elseif (stripos($_SERVER['REQUEST_URI'],'resend-confirmation')!== false) {
		...
		$gsom_form_vars['gsom_email_field'] = $email;
				...
				if ( $present === GSOM_SS_SUBSCRIBED ) {
					...
					gsom_redirect( gsom_BuildLink('already-confirmed') );
				} else {            
						gsom_redirect( gsom_BuildLink('confirmation-failed') );
				}
		...
		{
			// Thanks to Mohib ( nanowisdoms @ http://justlan.com/forums ) for the fix
			extract(gsom_RequestParamsPresent('u'));
			$email = gsom_get_email_by_ucode($u); 
			gsom_init_gvars_with_email($email,$u);
...
function gsom_put_form($print = true)
{
	global $gsom_form_vars;
		...
		if ( get_option('gsom_support_link') == 1 ) {
			$nofollow = is_page() ? '' : 'rel="nofollow"';
			$data .= '<p style="font-size:x-small; line-height:1.5em;">Newsletter plugin by <a '.$nofollow.' style="color: inherit;" href="http://www.glockeasymail.com/">glockeasymail.com</a></p>';
		}
...
function gsom_plugin_loaded(){    
		wp_register_sidebar_widget('gsom-widget','G-Lock Double Opt-in Manager', 'gsom_widget');        
}
...
// just the excerpt
function gsom_first_n_words($text, $number_of_words) {
...
function gsom_get_fancy_excerpt($maxLength = 350) {
	...
	return gsom_first_n_words($excerpt, $maxLength).'(...)';
}
...
function gsom_Mail($subjTpl, $messageTpl, $notification = false, $opts = false) {   		
	...
	try {
		...
		$x = 'PHPMailer error: '.$e->getMessage();
		gsom_log('PHPMailer error: '.$e->getMessage());
	}
	return $x;
}
...
function gsom_unconfirmed_cleanup() {
		global $wpdb;
		global $gsom_table_subs;    
		$sql = 'DELETE FROM '.$gsom_table_subs.' where intStatus = '.GSOM_SS_SUBSCRIBED.' and DATE_ADD(dtTime, INTERVAL 7 DAY) <= CURDATE()';
		$result = $wpdb->query($sql);
} 
//// dashboard widget
...
function gsom_put_deprecation_warning() {
	if ( isset($_REQUEST['page']) && in_array($_REQUEST['page'], array( 'gsom-optin', 'gsom-subscribers', 'gsom-broadcast' ) )  ) {
		?>
		<div id="message" class="updated"><p>'G-Lock Double Opt-in Manager' is discontinued and replaced by <a href="http://wpnewsman.com">WPNewsman</a>.</p></div>
		<?php
	}
}
...
if(!defined('GSOM_INCLUDE')) { // we don't need this code if the file was included from an ajax backend    
	...
	add_action('in_admin_header', 'gsom_put_deprecation_warning');
